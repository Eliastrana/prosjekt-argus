...
**Rutenettet må stemme eksakt.** `switch_graph` er `null`, så modellen bruker grafen som ligger i sjekkpunktet, med faste nodeposisjoner. MEPS-datasettet må da ha samme domene, projeksjon og utstrekning som MET brukte. N320 er trygt fordi det er et standard rutenett; MEPS er MET sitt eget. Utstrekningen 949 × 1069 er utledet bakover fra `CRPSFFTLoss` og er ikke oppgitt noe sted.

Men det samme forholdet som gjør dette til en hard begrensning, løser det også: grafen *ligger* i sjekkpunktet, og en Anemoi-graf bærer eksplisitte lat/lon-koordinater for hver node. Rutenettdefinisjonen er altså allerede i hus, og kan leses ut direkte fremfor å spørres om eller gjettes.
...
Første steg er å lese koordinatene ut av sjekkpunktet og se om de bekrefter eller avkrefter 949 × 1069. Det koster ingenting og gir det autoritative svaret, siden det er nettopp de nodene modellen indekseres mot.

Deretter kan MEPS-oppskriften ta form mot det offentlige arkivet på thredds.met.no, der NetCDF-headerne uansett bærer projeksjon og utstrekning.
